...
app.post('/api/create-order', async (req,res)=>{
  try {
    ...
    const name=clean(customer.name,100), phone=normalizePhone(customer.phone), email=clean(customer.email,150), address=clean(customer.address,600), startDate=clean(customer.startDate,20), note=clean(customer.note,500), password=String(customer.password||'');
    if(!name || !phone || !address || !startDate || !passwordOk(password)) return res.status(400).json({error:'Please fill all details and create a password of at least 6 characters.'});
    if(!phoneOk(phone)) return res.status(400).json({error:'Please enter a valid mobile number.'});
    const existingCustomer=readCustomers().find(c=>c.phone===phone);
    if(existingCustomer && !verifyPassword(password,existingCustomer.passwordSalt,existingCustomer.passwordHash)) return res.status(401).json({error:'This mobile number already has an account. Please use the correct account password.'});
...
app.post('/api/verify-payment', async (req,res)=>{
  try {
    ...
    const customers=readCustomers();
    const existingCustomer=customers.find(c=>c.phone===normalizePhone(orderDetails.phone));
    const suppliedPassword=String(orderDetails.password||'');
    if(!existingCustomer && passwordOk(suppliedPassword)){ const hp=hashPassword(suppliedPassword); customers.push({phone:normalizePhone(orderDetails.phone),name:clean(orderDetails.name,100),email:clean(orderDetails.email,150),passwordHash:hp.hash,passwordSalt:hp.salt,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}); saveCustomers(customers); }
    if(orderDetails && 'password' in orderDetails) delete orderDetails.password;
    saveOrder(order);
